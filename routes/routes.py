...
def getPredDataMonthlyjdvvnl():
    try:
        ...
        df1['load'] = float(df1['consumed_KWh'].astype(float)/24)
        ...
        if todos_pred:
            ...
            for i in range(len(todos_pred)):
                ...
                formatted_data_pred["data_pred"].append({"clock": date2, "pre_kwh": float(pred_daily_sum, 2), "pre_load": float(pred_daily_sum/24, 2)})
